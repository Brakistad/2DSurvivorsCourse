...
@export var link_ability: PackedScene
...
func on_timer_timeout():
	...
	var link_instance = link_ability.instantiate() as LinkAbility
	var foreground_layer = get_tree().get_first_node_in_group("foreground_layer")
	foreground_layer.add_child(link_instance)
	link_instance.hitbox_component.damage = base_damage * additional_damage_percent
	
	link_instance.global_position = enemies[0].global_position
	link_instance.global_position += Vector2.RIGHT.rotated(randf_range(0, TAU)) * 4
	
	var enemy_direction = enemies[0].global_position - link_instance.global_position
	link_instance.rotation = enemy_direction.angle()
...
func on_ability_upgrade_added(_upgrade: AbilityUpgrade, _current_upgrades: Dictionary):
	pass
